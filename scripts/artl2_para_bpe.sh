...
########################################################
KEY=artl2-para
rawfile_key=artl2-para                                   # name identifier for raw data in the format "train.${rawfile_key}.en"
RAW_SOURCE=/export/c11/sli136/l2mt/en-es/data/raw/${rawfile_key}             # raw (untokenized) data (artificial mixed data directory, doesn't include l2 and ref data)
DEST_DIR=/export/c11/sli136/l2mt/en-es/data/input_data_${KEY}  # parent directory for all tokenized data to store input for training
...
MODEL_DIR='/export/c11/sli136/l2mt/en-es/models/'${KEY}          # parent directory for all models to store output from training
