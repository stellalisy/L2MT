...
trans_output = args.target_output + '.' + TARGET_LANG
...
def get_report(all_stats):
    return [
        ...
        f"Corresponding {TARGET_LANG} translations written to {trans_output}",
...
def process_batch(batch: list[tuple[str]]):
    ...
    p = args.probabilities[args.errors.index("runon_comb")] if "runon_comb" in args.errors else 0
    ...
    for line_i, (tokens, line_es) in enumerate(tokenized_batch_good):
        ...
        for sent_id in append_order:
            ...
            sent_claus = process_line(tok_en)[0] if args.compound_errors else tok_en
        ...
        if num_inline > 0 or num_append > 0:
            all_stats.num_runOn_comb += num_written              # number of sentences per line (under-estimate)
            all_stats.num_inline_chg += int(num_append == 0)     # number of sentences that only considers in-line edits
...
def process_line(tokens: list[str], before=False):
    ...
    tokens, d = runOn_line(tokens)
    all_stats.num_runOn_line += d
    
    return tokens, d+d1+d2
...
f_es = open(args.source_file + '.' + TARGET_LANG, "r")
batch = []
